...
export default async function Home() {
  ...
  const [approvedRestaurants, restaurantCount, rating, reviewCount, cuisineGroups, cityGroups] = await Promise.all([
    ...
    prisma.restaurant.groupBy({
      by: ["cuisine"], where: { ...approvedWhere, cuisine: { not: "" } }, _count: { _all: true },
      orderBy: { _count: { cuisine: "desc" } }, take: 8,
    }),
    prisma.restaurant.groupBy({
      by: ["city"], where: { ...approvedWhere, city: { not: "" } }, _count: { _all: true },
      orderBy: { _count: { city: "desc" } },
    }),
  ]);

  const featuredRestaurants: RestaurantSummary[] = approvedRestaurants.map((restaurant) => ({
    id: restaurant.id, name: restaurant.name, slug: restaurant.slug, cuisine: restaurant.cuisine,
    city: restaurant.city, rating: restaurant.rating, reviewCount: restaurant.reviewCount,
    priceLevel: restaurant.priceLevel, description: restaurant.description,
    imageUrl: restaurant.images[0]?.url ?? restaurant.imageUrl, address: restaurant.address,
  ...
  return (
    <div className="overflow-hidden">
      <section className="relative mx-auto w-full max-w-7xl px-4 pb-14 pt-12 sm:px-6 md:pt-16 lg:px-8 lg:pb-20">
        ...
        <div className="max-w-4xl space-y-7">
          ...
          <div className="space-y-5">
            ...
            <p className="max-w-xl text-base leading-7 text-muted-foreground sm:text-lg sm:leading-8">Discover restaurants, cafés, and local food spots that turn an ordinary outing into a great story.</p>
          </div>
      ...
      <section id="cuisines" className="mx-auto w-full max-w-7xl px-4 py-18 sm:px-6 lg:px-8 lg:py-24">
        <div className="mb-9 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div className="max-w-xl space-y-3"><p className="text-sm font-semibold uppercase tracking-[0.24em] text-primary">Explore by cuisine</p><h2 className="text-3xl font-semibold tracking-[-0.03em] sm:text-4xl">Follow your cravings.</h2><p className="leading-7 text-muted-foreground">Browse the cuisines currently available from local restaurants.</p></div><Button asChild variant="outline" className="w-fit rounded-xl"><Link href="/restaurants">Browse all cuisines <ArrowRight className="h-4 w-4" /></Link></Button></div>
        {cuisines.length ? <div className="grid grid-cols-2 gap-4 sm:grid-cols-4 lg:grid-cols-8">{cuisines.map((cuisine) => <Link key={cuisine.name} href={{ pathname: "/restaurants", query: { cuisine: cuisine.name } }} className="group rounded-2xl border border-border bg-card p-4 text-center shadow-sm transition hover:-translate-y-1 hover:border-primary/25 hover:shadow-lg"><UtensilsCrossed className="mx-auto h-6 w-6 text-primary" /><p className="mt-5 text-sm font-semibold text-foreground transition-colors group-hover:text-primary">{cuisine.name}</p><p className="mt-1 text-xs text-muted-foreground">{cuisine.count} {cuisine.count === 1 ? "restaurant" : "restaurants"}</p></Link>)}</div> : <EmptyState> Cuisine categories will appear when restaurants are published.</EmptyState>}
      </section>

      <section id="explore" className="bg-[#fffaf3] py-18 lg:py-24"><div className="mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8"><div className="mb-9 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div className="max-w-xl space-y-3"><p className="text-sm font-semibold uppercase tracking-[0.24em] text-primary">Featured restaurants</p><h2 className="text-3xl font-semibold tracking-[-0.03em] sm:text-4xl">Places diners keep coming back to.</h2><p className="leading-7 text-muted-foreground">Current restaurant listings, selected from the platform.</p></div><Button asChild variant="outline" className="w-fit rounded-xl"><Link href="/restaurants">View all restaurants <ArrowRight className="h-4 w-4" /></Link></Button></div>{featuredRestaurants.length ? <RestaurantGrid restaurants={featuredRestaurants} /> : <EmptyState>New restaurant recommendations are arriving soon.</EmptyState>}</div></section>
